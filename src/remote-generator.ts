...
async function run() {
  ...
  updateTokenInfo(
    './src/tokenlists/hyperlock/overwrites/blast.ts',
    remoteTokens
  )
}
...
async function fetchRemoteData() {
  const res = await fetch(
    'https://raw.githubusercontent.com/ThrusterX/thruster-token-list/main/chains/81457/token-list.json'
  )
...
async function compareToLocal() {
  const localData = await JSON.parse(
    fs.readFileSync('./generated/hyperlock.tokenlist.json', 'utf8')
  )
  const remoteData: ThrusterTokenInfo[] = await fetchRemoteData()
  const localAddresses: string[] = localData.tokens.map(
    (token: Partial<TokenInfo>) => token.address?.toLowerCase()
  )

  const missingAddresses = remoteData.filter(
    ({ tokenAddress }) => !localAddresses.includes(tokenAddress.toLowerCase())
  )
...
async function getIcons(newTokens: TokenInfo[]) {
  const imageUrls = newTokens.map(({ address }) => ({
    address: address,
    url: `https://raw.githubusercontent.com/ThrusterX/thruster-token-list/main/chains/81457/assets/${address}/token-logo.svg`,
  }))
...
const updateTokenList = (path: string, tokens: TokenInfo[]) => {
  ...
  tokenList.splice(
    tokenList.length - 2,
    0,
    ...tokens.map(({ address, name }) => `\t'${address}', // ${name}`)
  )
...
const updateTokenInfo = (path: string, tokens: TokenInfo[]) => {
  ...
  for (const token of tokens) {
    const newTokenInfo = generateTokenInfo(
      token.address,
      token.name,
      token.symbol
    )
